...
# agentes: claude · codex · gemini · hermes · openclaw · cursor · copilot · universal
...
usage(){ sed -n 2,6p "$0" 2>/dev/null | sed 's/^# \{0,1\}//' || true
  echo "uso: install.sh <claude|codex|gemini|hermes|openclaw|cursor|copilot|universal> [--project [PASTA]]"; exit 1; }
[ -z "$AGENT" ] && usage
...
case "$AGENT" in
  ...
  openclaw)  VAR="$SRC/agents/openclaw/jsmotion";  U="${OPENCLAW_STATE_DIR:-$HOME/.openclaw}/skills/jsmotion"; P="$PROJ/skills/jsmotion" ;;
...
case "$AGENT" in
  ...
  openclaw) echo "   No OpenClaw: /new (ou openclaw gateway restart) e peça um vídeo animado — ou /jsmotion."
            [ -d "$HOME/.agents/skills/jsmotion" ] && echo "   Aviso: ~/.agents/skills/jsmotion também existe e tem prioridade no OpenClaw (funciona, mas usa aquela variante)." ;;
