...
## Neste agente: Codex (CLI, IDE e app)

- **`$SK`** = a pasta desta SKILL.md — o Codex mostra o caminho na lista de skills (ex.: `~/.agents/skills/jsmotion`). Rode `export SK=<pasta>` no primeiro comando.
- **Perguntas**: o Codex não tem botões de opção. Escreva **uma** pergunta com as opções numeradas, a sua
